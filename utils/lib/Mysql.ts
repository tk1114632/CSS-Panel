...
db.on('connection', async (connection) => {
	...
	try {
		// Create basic tables (no await needed for simple CREATE statements)
		connection.query(
		...
		// Use db (pool) instead of connection for complex queries
		// Check and add columns to sa_mutes table if they don't exist
		const [checkMutesColumnsResult] = await db.query(
		...
		const [checkBansColumnsResult] = await db.query(
